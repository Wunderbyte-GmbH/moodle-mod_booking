...
* Bugfix: The onwaitinglist availability condition reads the type 2 ("open after full pass") state through db_waitlist_offer_repository::is_open_mode_active() instead of the raw booking_options.waitlistopenmode field, so the condition and the waiting list progression agree on when the open mode is active. (Wunderbyte-GmbH/moodle-mod_booking#1506)

* New feature: New setting "Create PDF/A-2b compliant PDFs" (booking/pdfaenabled, off by default). When enabled, the sign-in sheet in HTML template mode (setting signinsheethtml) and the checklist (setting checklisthtml) are generated as PDF/A-2b (archivable): all fonts are embedded (Helvetica/Times/Courier in templates are mapped to FreeSans/FreeSerif/FreeMono), CMYK images are converted to RGB, images referenced by URL are downloaded by the server (Moodle's cURL security settings apply) and left out when they cannot be loaded. When disabled the PDFs are generated exactly as before. Requires local_wunderbyte_table 3.3.3 (2026081800).

* Improvement: Less jQuery - the inline editor of booking notes (edit_note), the booking instance template selector (bookinginstancetemplateselect) and the sync source selector (form_sync_source_selector) use plain DOM APIs now. jQuery remains where it is inherent: the rating widget (a jQuery plugin) and its caller, the Bootstrap 4 collapse() call of the performance chart, and the bundled Vue app.

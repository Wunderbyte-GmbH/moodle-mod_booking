...
* Improvement: The extra columns for dates of the sign-in sheet ("Add extra columns for dates", HTML template and classic mode) are labelled "August 27th" / "27. August" and show the start time whenever the booking option has more than one session on the same day ("Aug. 27th, 3:00 pm" / "27. Aug., 15:00"), so the columns can be told apart (new language strings signinsheetcolumndate and signinsheetcolumndatetime); the labels are centered in the column. In classic mode two same-day sessions were additionally collapsed into a single column because their labels were identical.

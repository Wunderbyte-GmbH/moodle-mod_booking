* Improvement: The HTML template of the sign-in sheet (setting signinsheethtml) resolves custom booking option fields and all placeholders of the booking rules outside of the [[users]] section now - written with double square brackets instead of curly braces, e.g. [[myshortname]], [[bookingoptionname]], [[startdate]] or [[numberparticipants]]; the setting lists them below its description. Which placeholders are available there is decided per placeholder class with for_signinsheet(): the option, instance and course related placeholders and the custom fields opt in, user and event related ones (e.g. [[firstname]], [[changes]]) stay unresolved outside of [[users]]. Custom user profile fields are available inside of [[users]] only, rendered per booked user (as before); outside of [[users]] they stay unresolved. The placeholders of the template mode ([[location]], [[dayofweektime]], [[teachers]], [[dates]], [[logourl]], [[tablename]]) keep their known values and nothing changes inside [[users]]. All [[...]] placeholders of the template are case-insensitive now ([[eXTraNaMe]] renders the profile field Extraname, [[BookingOptionName]] the option name). Curly braces of the template itself (css rules) are left alone, unknown placeholders stay visible as before, and the data of the booked users is inserted only after all placeholders were resolved, so it is never parsed for placeholders.
* Improvement: placeholders_info::return_list_of_placeholders() selects the list with the new constants MOD_BOOKING_PLACEHOLDERS_ALL, MOD_BOOKING_PLACEHOLDERS_POLLURL and MOD_BOOKING_PLACEHOLDERS_SIGNINSHEET instead of a bool, so further placeholder subsets can be added easily (passing true still yields the poll url list). Placeholder classes opt in to the sign-in sheet list with for_signinsheet() (default false, like for_pollurl() for the poll url list).
...
* Bugfix: placeholders_info::return_list_of_placeholders() served the reduced list of the poll url placeholders as the full list of placeholders when the reduced list had been built first in the same request (the static cache ignored the pollurl flag) - e.g. on the plugin settings page of PRO sites, where the poll url template is rendered before the sign-in sheet template. Both lists are cached separately now.
* Bugfix: Custom user profile fields inside [[users]] of the sign-in sheet HTML template (and the custom profile field columns of the legacy PDF) were empty on PostgreSQL whenever the shortname contains uppercase letters, because the value was selected under the shortname as column alias and PostgreSQL folds unquoted aliases to lowercase; on MySQL 8 a shortname that is a reserved word (e.g. rank) broke the query. The values are selected under an alias derived from the field id now, with the field id as bound parameter instead of a LIKE on the shortname.
* Test: New PHPUnit tests for placeholders in poll urls ({pollurl} and {pollurlteachers} rendered per user, the poll url mode of render_text() replacing only the placeholders of the poll url list, custom booking option fields in poll urls, url encoded braces, the PRO gate) and for the placeholder lists and the sign-in sheet template placeholders.

* Improvement: The entry ticket can be shown as a download button with a ticket icon in the booking options overview (list and cards): "Ticket (download button)" is selectable under "Customize columns and fields" > "Bookings overview - page" of the booking instance once entry tickets are enabled. The button only appears for users holding a valid ticket for the option.

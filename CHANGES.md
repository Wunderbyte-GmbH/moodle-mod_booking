## Version 9.7.8 (2026082807)
* Bugfix: Every upgrade step of the waiting list refactoring and of the stable customform element identity is anchored above the base version now (2026082803 - 2026082807). This branch sits on the USI line, which released 2026082801 itself, so the previous anchors (2026081700 - 2026082101) were below that base: a site coming from there skipped the creation of booking_waitlist_offers and booking_waitlist_declines, the legacy chain migration and the customform elementid migration, while still reporting the higher version. The new anchors start at 2026082803 rather than 2026082802, because 2026082802 was already handed out for the waitlistopenmode step and sites that ran it would otherwise skip the table creation now sitting on that number. All steps are guarded (table and field existence checks, idempotent migrations), so sites that already ran them pass through again. (Wunderbyte-GmbH/moodle-mod_booking#1506)

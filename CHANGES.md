...
* Improvement: The bookings tracker (report2.php) is a regular feature now - no PRO license required anymore. It stays gated by its admin setting (bookingstracker, default off for existing sites); the settings block moved out of the PRO section and the navigation nodes lost their PRO badge/disabled styling. The setting gate on the page itself now really blocks access (before, the warning was only prepended and the page rendered anyway).

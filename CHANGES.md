* Bugfix: Ticket PDFs are served without browser caching. They were sent with a private six-hour cache lifetime, so a browser used by several accounts in turn could hand the cached PDF of one user to the next one logged in without asking the server. The download decision moved from lib.php into ticket_manager::can_download() (holder, "View ticket report", or entry staff of the option).
* Test: The sticky save/cancel bars of the booking option form and the booking instance form (and the sticky "you are editing" alert) stay in the flow on Behat sites, like core does for its sticky footer. The driver does not account for sticky elements when scrolling a field into view, so the bar covered the "Ticket design" field and intercepted the click (Moodle 5.1, Chrome).
* Improvement: mod_booking's own JavaScript no longer uses jQuery. The rating stars of the options overview run on a new dependency-free widget (mod_booking/starrating) that keeps the look and adds keyboard and screen reader support; the bundled jQuery Bar Rating plugin was removed. view_actions, performance_chart, the inline editor of booking notes, the booking instance template selector and the sync source selector use plain DOM APIs. The bundled Vue app (app-lazy) read the page language through jQuery; it uses the DOM now, so no AMD module of the plugin depends on jQuery any more.
* Bugfix: The ticket scanner follows the option's current "Personalised ticket" setting instead of the flag stored on the ticket when it was issued, so tickets issued before an option was switched to transferable are checked in automatically too.
* Improvement: Ticket scanner - for booking options with dates a check-in only sets the presence of the scanned date; the presence status of the whole booking is no longer touched (options without dates still set it on the booking). Tickets that are neither personalised nor subject to an identity confirmation are checked in automatically on scan: the green screen shows for two seconds (or until "Next" is pressed) and the camera returns. The last successful scan is shown below the camera. New webservice parameter autocheckin and return values autocheckin / autocheckedin on mod_booking_verify_ticket.
* Bugfix: The "Manage responses" page showed an empty presence cell for participants checked in with the ticket scanner: its presence column only knew the statuses up to "Excused", so "Checked in" was stored but not displayed. The column now uses the common list of presence statuses.
* Bugfix: In the ticket scanner "Confirm entry" and "Reject" did nothing after the first scan (no request was sent), because finally() was called on the jQuery promises returned by core/ajax.
* New feature: The ticket scanner is started per booking option (entry "Ticket scanner" in the option's action menu, or a scanner button for entry staff without editing rights) and then only accepts tickets of that option; a ticket of another event is refused with a red screen naming the event. The option's dates are offered before the first scan. The instance-wide scanner stays and moved into the "More" menu of the booking instance. New webservice parameter expectedoptionid on mod_booking_verify_ticket / mod_booking_reject_ticket (statuses wrongoption, closed).
* New feature: Who may scan is decided in one place (ticket_manager::can_scan()): holders of "Scan entry tickets", or users picked as "Additional entry staff" in the option's "Ticketing" section. The picker only offers users whose profile the editor may view (a manager sees everyone); staff a manager picked survive a teacher's save. New webservice mod_booking_search_ticketscanners.
* New feature: Availability window of the scanner per option: "Scanner available before / after each date" in the "Ticketing" section (unlimited by default, options without dates are always available). Outside the window the scanner page is blocked with the next opening time and the webservices refuse with status "closed".
* Improvement: The entry ticket can be shown as a download button with a ticket icon: "Ticket (download button)" is selectable under "Customize columns and fields" of the booking instance for "Bookings overview - page" (list and cards, the viewer's own ticket) and for "Manage responses page & Bookings tracker" (the participant's ticket, on report.php and in the bookings tracker) once entry tickets are enabled. In the overview a booking made before the ticket design was chosen gets its ticket created on first sight; participant lists only show tickets that exist.
* New feature: Entry tickets are checked in per date. For booking options with several dates the scanner records the presence for the date being admitted (stored per date in the bookings tracker, plus on the booking answer on the first admission), defaulting to the nearest date - the session running right now, else the next upcoming one - with a sticky date selection per option in the scanner. A date checked in with the scanner is counted by the tracker's presence counter as soon as the counted presence status equals the status set on check-in; the ticket settings warn when the two differ. The webservice mod_booking_verify_ticket gained the parameter optiondateid and returns the dates of the option, the selected date and the per-date presence.
* New feature: Full-screen scanner result. Every scan covers the page with a colour-coded result (animated check mark for a valid ticket), a big "Confirm entry" button and a "Reject" button; the check-in is only written on confirmation, a rejection writes no presence but is logged as the new event "Entry ticket rejected" (usable in booking rules) via the new webservice mod_booking_reject_ticket. The new site setting "Identity data shown on scan" selects the holder data shown for personalised tickets and options requiring an identity confirmation - profile picture, name, e-mail and any custom profile field such as birth date or gender.
* Bugfix: Unticking "Personalised ticket" in the option form is saved now; before, the flag fell back to its default (personalised) because the unticked value was dropped.
* New feature: Entry tickets (SofaTicket) for booking options: personalised PDF entry tickets with a QR code, created on booking, delivered by a booking rule and verified and checked in at the door. tool_certificate is used as a layout engine only — a ticket is a booking_tickets record with its own file area, so creating a ticket fires no certificate events, sends no tool_certificate notification and does not appear in the certificate report. The site administration only switches the feature on (plus the site-wide scanner defaults); the ticket design and behaviour are configured per booking option in the new "Ticketing" section of the option form (design, personalised, require identity confirmation, additional ticket information). Includes the new event ticket_created and the rule action "Send ticket", which mails the ticket PDF and resolves the ticket at run time (so it also works on other events, e.g. to send the ticket a few days before the start), the placeholders {ticketurl}, {ticketcode} and {ticketverifyurl}, "My tickets" (mytickets.php, a profile node and a column in My bookings), race-safe ticket creation and check-in, GDPR coverage and ticket cleanup on option/instance deletion, and agent skills for ticketing. Requires an upgrade run (new table booking_tickets).
...
## Version 9.7.10 (2026090301)
* New feature: New site setting "Course login required to edit booking options" (editoptionsrequirecourselogin, enabled by default). When disabled, a site login is enough to open the booking option form (editoptions.php), e.g. for external trainers who are not enrolled in the course. The capability checks of the form still apply.
* New feature: Booking instance duplication copies the connected Moodle courses. With the setting "Duplicate Moodle course" active, duplicating a booking instance copies every connected course as well and points the duplicated options at the copies; options which shared one course keep sharing its single copy. The copy itself runs in the core adhoc course copy task, so a course shell is linked immediately and filled in on the next cron run. On a restore onto another site nothing is copied: an option connected to the course the backup came from is connected to the course being restored into, any other connection is dropped instead of silently pointing at whatever course carries that id there. (Wunderbyte-GmbH/Wunderbyte-GmbH#2193)
* New feature: Naming scheme for connected Moodle courses (PRO). Three new settings define templates for the full name, the short name and the ID number of a course that mod_booking creates or copies for a booking option, e.g. {titlewithoutprefix}_{optionid}. Short names and ID numbers are kept unique. The new placeholder {titlewithoutprefix} renders the option title without its prefix and, unlike {title}, does not need a logged in user, so it also works in background tasks. Courses the user picked from the list are never renamed. (Wunderbyte-GmbH/Wunderbyte-GmbH#2193)
* Bugfix: The naming scheme survives the asynchronous course copy. Core's copy task rewrote the full name, short name and ID number of the copy on the next cron run; a new adhoc task re-applies the naming once the copy has settled. Two options created from the same template with the same title both end up with a course named exactly after the option. (Wunderbyte-GmbH/Wunderbyte-GmbH#2193)
* Bugfix: Duplicating a booking instance or option no longer starts an endless chain of course copies. The connected course copy ran in every restore, including the restore performed by the course copy it had just queued, so a course containing a booking instance whose option enrolled into that very course - or two courses referencing each other - was copied again on every cron run. Connected courses are now only copied when a single booking instance is duplicated, never by a course copy, course restore or course import, and an option enrolling into the course its booking instance lives in keeps that connection. (Wunderbyte-GmbH/Wunderbyte-GmbH#2193)
* Improvement: The course copy no longer depends on the acting user - the backup and restore are attributed to the admin user, so it also works from adhoc tasks and restore steps; the capability check for duplicating a booking option is done at the call site. (Wunderbyte-GmbH/Wunderbyte-GmbH#2197)
* Bugfix: The privacy export no longer fails. (Wunderbyte-GmbH/moodle-mod_booking#1517)
* Bugfix: The rule condition "responsible contact" mailed the responsible contacts of every booking option on MySQL/MariaDB instead of the contacts of the affected option only. (Wunderbyte-GmbH/moodle-mod_booking#1547)
* Bugfix: The first form save of an option created outside the option form (CSV import, web service, wizard) no longer records phantom changes and no longer notifies booked users about a change that never happened: dates are compared with minute precision, editor markup around plain text is not a change, and an empty text is not a change against a missing one. (Wunderbyte-GmbH/moodle-mod_booking#1556)
* Test: Behat suite sped up and stabilised - preconditions are seeded through generators instead of forms, hard waits, redundant logins and duplicate scenarios removed, cron and log round trips replaced by an adhoc run and an events log step. (Wunderbyte-GmbH/moodle-mod_booking#1551)
* Bugfix: With the setting "Use SQL filter for availability" (usesqlfilteravailability) enabled, trainers now see the booking options they teach on their teacher page, in the "Options I teach" tab and in the trainer course list shortcode, even if they do not fulfil the availability conditions of these options themselves. Visitors of a teacher page who are not that teacher still get the filtered list.

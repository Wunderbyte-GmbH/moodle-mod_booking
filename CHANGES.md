## Version 9.7.9 (2026083101)
* Bugfix: The adhoc task purge_campaign_caches no longer dies on an orphaned booking option (a row in booking_options whose bookingid has no course module left, e.g. after a deleted instance). get_coursemodule_from_instance() returned false there and singleton_service handed null to booking_settings::__construct(), which raised a TypeError - a \Error, so neither the catch in the singleton service nor the one in the task loop caught it. The whole campaign transition aborted at the first such option: every option after it got neither sync_waiting_list() nor the bookingoption_freetobookagain event, and the failed task was retried with a growing faildelay, purging the option, settings and price caches site-wide on every attempt. The singleton service returns null for a missing course module now, the task skips a broken option instead of aborting, and it reads the ids and maxanswers of the options with limited places in one query restricted to options that still have a course module (one query per option less). (Wunderbyte-GmbH/moodle-mod_booking#1588)

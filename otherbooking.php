...
echo $OUTPUT->header();
echo $OUTPUT->heading(
    get_string("editotherbooking", "booking") . " [{$option->option->text}]",
    3,
    'helptitle',
    'uniqueid'
);
...
foreach ($rules as $rule) {
    ...
    $buttons = $OUTPUT->render_from_template('mod_booking/button_row', [
        ...
        'buttons' => [
            (new single_button($edit, get_string('editrule', 'booking'), 'get'))->export_for_template($OUTPUT),
            (new single_button($delete, get_string('deleterule', 'booking'), 'get'))->export_for_template($OUTPUT),
        ],
    ]);

    $rulestable[] = ["{$rule->text}", $rule->userslimit, $buttons];
}
...
echo $OUTPUT->render_from_template('mod_booking/button_row', [
    ...
    'buttons' => [
        ...
        (new single_button($addnew, get_string('otherbookingaddrule', 'booking'), 'get'))->export_for_template($OUTPUT),
    ],
]);

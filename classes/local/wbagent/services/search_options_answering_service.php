...
class search_options_answering_service {
    ...
    /**
     * Build a grounded prompt for concise search result summaries.
     *
     * @param string $question
     * @param string $query
     * @param string $when
    ...
    private function build_prompt(
        ...
        $userquestion = trim($question) !== ''
            ? trim($question)
            : (trim($query) !== '' ? trim($query) : 'Find booking options.');

        $optionlines = [];
        foreach (array_slice($options, 0, 8) as $option) {
            $name = trim((string)($option['name'] ?? ''));
            $id = (int)($option['id'] ?? 0);
            $link = trim((string)($option['link'] ?? ''));
            if ($name === '') {
                continue;
            }
        ...
        $optionsection = empty($optionlines) ? '- none' : implode("\n", $optionlines);
        $querytext = trim($query) !== '' ? trim($query) : '(empty query)';
        $whentext = trim($when) !== '' ? trim($when) : '(none)';

        return "You are a helpful assistant for the Moodle Booking plugin.\n"
            . "Summarize booking option search results based only on the provided data.\n\n"
            . "Rules:\n"
            . "- Output plain text only.\n"
            . "- Use concise wording and short bullet points when useful.\n"
            . "{$languagerule}\n"
            . "- Keep the answer at or below 650 characters.\n"
            . "- If no options are found, clearly say so.\n"
            . "- If options are found, mention the total and highlight the most relevant ones.\n"
            . "- Do not invent options, links, or attributes that are not in the data.\n"
            . "- Do not mention prompts, JSON, or internal implementation details.\n\n"
            . "User question:\n{$userquestion}\n\n"
            . "Search query: {$querytext}\n"
            . "When filter: {$whentext}\n"
            . 'Result count: ' . count($options) . "\n"
            . "Options:\n{$optionsection}";
    }
}

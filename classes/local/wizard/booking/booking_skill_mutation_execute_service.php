...
class booking_skill_mutation_execute_service {
    ...
    public function execute(string $taskname, array $input, int $cmid, int $userid, booking_skill_support $support): ?array {
        ...
        if (!$cm) {
            return [
                'status' => 'error',
                'detail' => get_string('agent_booking_invalid_course_module', 'bookingextension_agent'),
                'resultid' => null,
            ];
        }
        ...
        if (
            ...
            if (empty($previewids)) {
                return [
                    'status' => 'error',
                    'detail' => get_string('agent_booking_bulk_update_no_preview', 'bookingextension_agent'),
                    'resultid' => null,
                ];
            }
            ...
            if ($this->is_self_reference_query($teacherquery) && !empty($USER->email)) {
                ...
                if ($teacherresult !== null) {
                    if (empty($teacherresult['email'])) {
                        return [
                            'status' => 'error',
                            'detail' => get_string('agent_booking_teacher_no_email', 'bookingextension_agent'),
                            'resultid' => null,
                        ];
                    }
        ...
        if (!empty($input['enrolledincoursequery'])) {
            ...
            if (empty($restrictioncourses['courseids'])) {
                return [
                    'status' => 'error',
                    'detail' => get_string('agent_booking_no_valid_course_enrolled', 'bookingextension_agent'),
                    'resultid' => null,
                ];
            }
        ...
        if ($taskname === create_option_skill::TASK_NAME) {
            ...
            if (empty($optionids)) {
                ...
                $detail = get_string('agent_booking_no_matching_options_to_update', 'bookingextension_agent');
                return [
                    'status' => 'error',
                    'detail' => $detail
    ...
    public function preflight_validate(string $taskname, array $input, int $cmid, int $userid): array {
        ...
        $cm = get_coursemodule_from_id('booking', $cmid);
        if (!$cm) {
            $errors[] = get_string('agent_booking_invalid_course_module', 'bookingextension_agent');
            return [
                'errors' => $errors,
        ...
        if ($this->is_update_option_style_task($taskname) && empty($normalizedinput['optionid'])) {
            ...
            if ($optionquery !== '') {
                if (booking_skill_support::is_last_preview_selection_reference($optionquery)) {
                    $previewids = booking_skill_support::resolve_last_preview_option_ids_for_user_for_execute($cmid, $userid);
                    if (empty($previewids)) {
                        $errors[] = get_string('agent_booking_bulk_update_no_preview', 'bookingextension_agent');

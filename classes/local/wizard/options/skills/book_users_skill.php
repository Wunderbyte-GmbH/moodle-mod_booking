...
class book_users_skill extends booking_skill_base implements
    ...
    public function __construct() {
        parent::__construct(false, \bookingextension_agent\local\wizard\dto\skill_risk_class::R3, ['mod/booking:bookforothers']);
    }
        ...
        if (!$confirmed && $resolvedoptionid > 0 && empty($issues)) {
            ...
            foreach ($bookuserids as $uid) {
                ...
                if (!empty($hardblockers)) {
                    ...
                    $issues = array_merge($issues, $this->build_preflight_issues([
                        get_string('agent_booking_user_cannot_book_hard_block', 'bookingextension_agent', (object)[
                            'userid' => $uid,
                    ...
                    $softoverridelines[] = get_string('agent_booking_book_users_soft_block', 'bookingextension_agent', (object)[
                        'userid' => $uid,
                        'descriptions' => $descriptions,
                    ]);
                }
            ...
            if (!empty($softoverridelines)) {
                $issues[] = [
                    ...
                    'user_question' => implode(' ', $softoverridelines)
                        . ' ' . get_string('agent_booking_book_users_soft_block_confirm', 'bookingextension_agent'),
                ];
            }
        }
    ...
    public function execute(array $input, int $cmid, int $userid): array {
        ...
        $detail = get_string('agent_booking_book_users_booked', 'bookingextension_agent', (object)[
        ...
        if ($userlinks !== '') {
            $detail .= ' ' . get_string('agent_booking_booked_users_label', 'bookingextension_agent') . ': '
                . $userlinks . '.';
        }

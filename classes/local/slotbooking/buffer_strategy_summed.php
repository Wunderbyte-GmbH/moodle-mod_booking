...
/**
 * Buffer combination strategy: adjacent buffers are stacked, not shared.
 *
 * @package mod_booking
...
/**
 * The cooldown of the earlier booking must fully elapse before the warmup of the later
 * booking may start. The required gap is therefore the sum of both buffers.
 */
class buffer_strategy_summed implements buffer_combination_strategy {
    /**
    ...
    public function required_gap(int $cooldownminutes, int $warmupminutes): int {
        return $cooldownminutes + $warmupminutes;
    }
}

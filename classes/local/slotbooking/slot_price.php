...
class slot_price {
    ...
    private static function get_base_slot_price_data(int $optionid, int $userid = 0): array {
        ...
        // Load-bearing fallback (do NOT remove): mod_booking\price::get_price() returns [] whenever the
        // user's resolved price category does not match a price record — which happens for ordinary
        // configurations, e.g. pricecategoryfallback=2 (empty category) or options that carry no 'default'
        // price record. A slot must still expose a base price in those cases, so we resolve the option's
        // default (or first) price record directly. Covered by slot_price_test::
        // test_fallback_provides_base_price_when_resolver_empty.

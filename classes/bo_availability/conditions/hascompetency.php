...
/**
 * Availability condition: User has a specific competency.
 *
 * @package mod_booking
...
/**
 * Availability condition: User has a specific (global) competency.
 *
 * @package mod_booking
 * @copyright 2025 Wunderbyte GmbH <info@wunderbyte.at>
 * @author Bernhard Fischer
...
class hascompetency implements bo_condition {
    ...
    public function is_available(booking_option_settings $settings, int $userid, bool $not = false): bool {
        ...
        if (empty($this->customsettings->competencyids)) {
            $isavailable = true;
        } else {
            ...
            if (empty($this->customsettings->competencyidsoperator) || $this->customsettings->competencyidsoperator != 'OR') {
                foreach ($competencyids as $competencyid) {
                    try {
                        ...
                        $hascompetency = false;
                        debugging('Exception in hascompetency condition: ' . $e->getMessage());
                    }
                ...
                foreach ($competencyids as $competencyid) {
                    ...
                    // We only get true, if the user has at least one of the competencies.
                }
            }
            $isavailable = $hascompetency;
        }

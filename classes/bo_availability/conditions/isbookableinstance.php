...
/**
 * Class to check if booking this option is disabled.
 *
 * @package mod_booking
 * @copyright 2022 Wunderbyte GmbH
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class isbookableinstance implements bo_condition {
    /** @var int $id Standard Conditions have hardcoded ids. */
    public $id = MOD_BOOKING_BO_COND_ISBOOKABLEINSTANCE;
    ...
    public function get_name(): string {
        return get_string('bocondisbookableinstance', 'mod_booking');
    }
    ...
    public function is_available(booking_option_settings $settings, int $userid, bool $not = false): bool {
        ...
        if (empty(booking::get_value_of_json_by_key((int) $settings->bookingid, "disablebooking"))) {
            $isavailable = true;
        }
    ...
    private function get_description_string(bool $isavailable, bool $full, booking_option_settings $settings) {
        ...
        if ($isavailable) {
            ...
            $description = $full ? get_string('bocondisbookablefullnotavailable', 'mod_booking') :
                get_string('bocondisbookablenotavailable', 'mod_booking');
        }
        return $description;
    }
}

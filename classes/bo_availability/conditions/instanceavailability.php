 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class instanceavailability implements bo_condition {
    /** @var int $id Standard Conditions have hardcoded ids. */
    public $id = MOD_BOOKING_BO_COND_INSTANCEAVAILABILITY;
    ...
    public function get_name(): string {
        return get_string('bocondinstanceavailability', 'mod_booking');
    }
    ...
    public function is_available(booking_option_settings $settings, int $userid, bool $not = false): bool {
        ...
        [$course, $cm] = get_course_and_cm_from_cmid($settings->cmid, 'booking');

        // If we don't have a course module, we cannot check it's availability.
        if (empty($cm)) {
            return true;
        }

        // Load availability info for the module.
        $info = new info_module($cm);
        $availabilityinfo = '';
        if ($userid == $USER->id) {
            // For the current user, we can use caching.
            $isavailable = $info->is_available($availabilityinfo, true);
        } else {
            // For users from cashier, we need to check the availability without caching.
    ...
    private function get_description_string(bool $isavailable, bool $full, booking_option_settings $settings): string {
        ...
        if ($isavailable) {
            $description = $full ? get_string('bocondinstanceavailabilityfullavailable', 'mod_booking') :
                get_string('bocondinstanceavailabilityavailable', 'mod_booking');
        } else {
            $urltoinstance = new moodle_url(
                '/course/modedit.php',
                ['update' => $settings->cmid],
                'id_availabilityconditionsheader'
            );
            $description = $full ? get_string('bocondinstanceavailabilityfullnotavailable', 'mod_booking', $urltoinstance->out()) :
                get_string('bocondinstanceavailabilitynotavailable', 'mod_booking');
        }
        return $description;
    }
}

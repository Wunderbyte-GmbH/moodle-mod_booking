...
class settings {
    ...
    /**
     * Prepares the data to be sent with emails.
     *
     * @param stdClass $option
     * @param stdClass $user
     * @param int $cmid Course module id
     * @param string $optiontimes Rendered sessions
     * @param array|false $changes An array containing changes
     * @return stdClass data to be sent via email
     */
    public function get_email_params(stdClass $option, stdClass $user, $cmid,
        $optiontimes = '', $changes = false, $issessionreminder = false, $includebookingdetails = false) {
        ...
        // TODO: find out which duration to use!
        ...
        if (!$issessionreminder) {
            ...
            $params->status = booking_get_user_status($user->id, $option->id, $this->id, $cmid);
            $params->participant = fullname($user);
            $params->email = $user->email;
            $params->title = format_string($option->text);
            $params->duration = $this->duration; // TODO: find out if this is the correct way to get duration!
            ...
            if (empty($option->pollurlteachers)) {
                $params->pollurlteachers = $this->pollurlteachers;
            } else {
            ...
            // Booking_option instance is needed to access functions get_all_users_booked and get_all_users_on_waitinglist.
        ...
        // These links should not be clickable (beacuse they will be copied by users), so style was changed appropriately.

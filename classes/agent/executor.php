...
use mod_booking\agent\interfaces\agent_executor;

/**
 * Dispatches interpreter-validated commands to the appropriate task provider.
 *
...
class executor implements agent_executor {
     ...
     * @return array
     */
    public function execute_commands(array $commands, int $cmid, int $userid, string $idempotencykey): array {
        ...
        // Idempotency guard.
        if ($this->store->run_exists($idempotencykey)) {
        ...
        foreach ($commands as $cmd) {
            ...
            if (!$provider) {
                $results[] = ['status' => 'error', 'detail' => "No provider for task '$taskname'.", 'resultid' => null];
                continue;
            }

            // Re-validate before execution (stale state detection).
            $validation = $provider->validate($taskname, $input, $cmid);
            if (!$validation['valid']) {
            ...
            $result = $provider->execute($taskname, $input, $cmid, $userid);
            $results[] = $result;
        }

    ...
    public static function return_value(
        ...
        if (
            ...
            if (!empty($eventdata->relateduserid)) {
                ...
                // Get date of course (of added teacher)
                $query = "
                SELECT bodt.id AS optiondateteacherid, bod.id AS optiondateid, -- bod.optionid, bod.bookingid, bod.reason, bod.reviewed, 
                    TO_CHAR(TO_TIMESTAMP(bod.coursestarttime), 'DD.MM.YYYY, HH24:MI') AS coursestarttime,
                    TO_CHAR(TO_TIMESTAMP(bod.courseendtime), 'DD.MM.YYYY, HH24:MI') AS courseendtime
                    -- , bodt.userid, bodtd.reason AS reason2, bodtd.timemodified
                FROM public.mdl_booking_optiondates bod
                LEFT JOIN public.mdl_booking_optiondates_teachers bodt
                ON bodt.optiondateid = bod.id
                LEFT JOIN public.mdl_booking_odt_deductions bodtd 
                    ON bodtd.optiondateid = bod.id
                WHERE bod.optionid = ?
                    AND bodt.userid = ?
                ORDER BY bodt.id DESC
                LIMIT 1;";
                $date = $DB->get_record_sql($query, [$eventdata->objectid, $eventdata->relateduserid]);
                $value = "$date->coursestarttime - $date->courseendtime";

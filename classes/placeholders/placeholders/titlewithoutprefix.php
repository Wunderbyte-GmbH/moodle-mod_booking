 ...
 * Returns the title of a booking option without the title prefix.
 *
 * In contrast to {title} and {bookingoptionname}, which both render the title
 * including the prefix (e.g. "BB42 - Beginner's course"), this placeholder
 * returns the plain title only (e.g. "Beginner's course").
 *
 * @copyright Wunderbyte GmbH <info@wunderbyte.at>
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class titlewithoutprefix extends \mod_booking\placeholders\placeholder_base {
    /**
    ...
    public static function return_value(
        ...
        // The title never depends on the user, so - unlike bookingoptionname - we
        // only require the optionid. This also keeps the placeholder usable from
        // contexts without a real user, e.g. adhoc tasks and restore steps.
        if (!empty($optionid)) {
            ...
            if (!empty($settings->text)) {
                $value = format_string($settings->text);
            }

 ...
 */
class select_deputy_of_supervisor implements booking_rule_condition {
    /** @var string $rulename */
    public $conditionname = 'select_deputy_of_supervisor';

    /** @var string $conditionnamestringid Id of localized string for name of rule condition*/
    protected $conditionnamestringid = 'selectdeputyofsupervisor';
    ...
    public function add_condition_to_mform(MoodleQuickForm &$mform, ?array &$ajaxformdata = null) {
        ...
        if (!empty($userfields)) {
            ...
            $mform->addElement(
                ...
                '',
                get_string('selectdeputyruledesc', 'mod_booking'),
            );
            ...
            $mform->addElement(
                ...
                get_string('rulecustomprofilefieldsupervisor', 'mod_booking'),
                $customuserprofilefieldsarray
            );

            $mform->addElement(
                'select',
                'fieldofuserfromeventdeputy',
                get_string('rulecustomprofilefieldofdeputy', 'mod_booking'),
                $customuserprofilefieldsarray
            );
    ...
    public function save_condition(stdClass &$data): void {
        ...
        $jsonobject->conditiondata->fieldofuserfromeventdeputy = $data->fieldofuserfromeventdeputy ?? '';
    ...
    public function set_defaults(stdClass &$data, stdClass $record) {
        ...
        $data->fieldofuserfromeventdeputy = $conditiondata->fieldofuserfromeventdeputy;
    ...
    public function execute(stdClass &$sql, array &$params): void {
        ...
        $supervisorfieldname = $data->conditiondata->fieldofuserfromevent ?? '';
        $deputyfieldname = $data->conditiondata->fieldofuserfromeventdeputy ?? '';

        $uiddataint = $DB->sql_cast_char2int('uid.data');
        $subsql = "
            SELECT DISTINCT depuid.data AS deputy_id
                FROM {user_info_data} depuid
                JOIN {user_info_field} depuif
                    ON depuid.fieldid = depuif.id
                WHERE depuif.shortname = :deputyfield
                AND depuid.userid IN (
                    SELECT $uiddataint
        ...
        $subparams = [
            'userid' => $userid,
            'supervisorfield' => $supervisorfieldname,
            'deputyfield' => $deputyfieldname,
        ];
        ...
        $useridsofsupervisors = array_filter(array_map('trim', explode(',', $rawvalue)));

        [$insql, $inparams] = $DB->get_in_or_equal($useridsofsupervisors, SQL_PARAMS_NAMED, 'uid');

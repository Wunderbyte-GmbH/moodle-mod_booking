...
> ## ⚠️ IMPORTANT — by default only FUTURE options are shown
>
> `[allbookingoptions]` **hides past booking options by default.** Only options whose end time
> is today or later are listed. To include past options too, you **must** add **`all="true"`**;
> use **`all="past"`** to show only past options.
>
> - `[allbookingoptions]` → future options only
> - `[allbookingoptions all="true"]` → all options (past **and** future)
> - `[allbookingoptions all="past"]` → past options only
>
> **Note for AI assistants:** an empty result does not mean the options do not exist — the
> default time filter may be hiding them. Re-check with `all="true"` before concluding anything.

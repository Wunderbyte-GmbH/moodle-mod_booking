...
function booking_restore_mods($mod, $restore) {
    ...
    if ($data) {
        ...
        $booking->sendmail = isset($info['MOD']['#']['SHOWINFO']['0']['#']) ? backup_todb(
                $info['MOD']['#']['SENDMAIL']['0']['#']) : '';
        ...
        $booking->pollurlteacherstext = backup_todb(
        ...
        if (!defined('RESTORE_SILENTLY')) {
            echo "<li>" . get_string("modulename", "booking") . " \"" .
                     format_string(stripslashes($booking->name), true) . "\"</li>";
        }
        backup_flush(300);
    } else {
        $status = false;
    ...
    if ($bookings = get_records_sql(
        ...
        foreach ($bookings as $booking) {
            ...
            if ($result != $content) {
                ...
                if (debugging()) {
                    if (!defined('RESTORE_SILENTLY')) {
                        echo '<br /><hr />' . s($content) . '<br />changed to<br />' . s($result) .
                                 '<hr /><br />';
                    }
                }
            }

...
final class bookingoptionimage_test extends advanced_testcase {
    ...
    public function test_headerimage_token_save_yields_non_null_source_and_imageurl(): void {
        ...
        $tokensvc = $this->attachment_token_service();
    ...
    public function test_preview_passthrough_scans_loop_results(): void {
        ...
        $passthrough = $this->engine_service_class('services\\preview_passthrough');
        $json = $passthrough::resolve_preview_json([], (int)$thread->id, '_test_loop_previews', $loopresults);
    ...
    /**
     * FQCN of an engine service class, resolved against the ACTIVE engine.
     *
     * Injected services must satisfy the active engine's interfaces, so naming one
     * engine's implementation here would break coexistence.
     *
     * @param string $relclass Class path below the engine's local\wizard namespace.
     * @return string
     */
    private function engine_service_class(string $relclass): string {
        return \mod_booking\local\wizard\engine\engine_resolver::fqcn($relclass);
    }
    ...
    private function attachment_token_service(): object {
        $class = $this->engine_service_class('services\\attachment\\attachment_token_service');
        return new $class();
    }
    ...
    private function thread_memory(): object {
        $class = $this->engine_service_class('services\\conversation_thread_memory');
        return new $class();
    }
}

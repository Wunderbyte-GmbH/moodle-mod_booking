...
Feature: Create global booking rules as admin and insure they are working.
  ...
  Scenario: Booking rules: create booking rule for booking course start time
    Given the following "mod_booking > rules" exist:
      | conditionname | conditiondata     | name       | actionname | actiondata                                                                     | rulename        | ruledata                                   |
      | select_users  | {"userids":["2"]} | 2daybefore | send_mail  | {"subject":"1daybefore","template":"will start tomorrow","templateformat":"1"} | rule_daysbefore | {"days":"1","datefield":"coursestarttime"} |

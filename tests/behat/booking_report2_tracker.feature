...
Feature: Use the bookings tracker (report2.php) as replacement of the old report.php
  ...
  Scenario: Booking report2: switch the system scope between aggregated options and single answers
    ...
    ## Rows are matched by content because the aggregated options share the same
    ## timecreated second, so their relative order is database-dependent.
    And I should see "2/10" in the "//tr[starts-with(@id, 'booked_system_0_r') and contains(., 'B2-Option1')]" "xpath_element"
    And I should see "2/5" in the "//tr[starts-with(@id, 'booked_system_0_r') and contains(., 'B1-Option1')]" "xpath_element"
    ...
    And I wait until the page is ready
    And I should see "2/10" in the "//tr[starts-with(@id, 'booked_system_0_r') and contains(., 'B2-Option1')]" "xpath_element"
    Then I should see "2/5" in the "//tr[starts-with(@id, 'booked_system_0_r') and contains(., 'B1-Option1')]" "xpath_element"

  @javascript
  Scenario: Booking report2: switch from system scope to option scope and view preconfigured additional fields
    Given I log in as "admin"
    And I visit "/mod/booking/report2.php"
    And "//tr[starts-with(@id, 'booked_system_0_r') and contains(., 'B2-Option1')]" "xpath_element" should exist

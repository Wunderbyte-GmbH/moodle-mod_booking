...
$users = $DB->get_records('user', [], 'id ASC');
\mod_booking\singleton_service::destroy_booking_option_singleton($optionid);
...
// Now, we reserve items unit there are no places available any more.
// In the parallel thread, users are constantly booked.
foreach ($users as $user) {
    ...
    // We book unitl all places are taken.
    if ($id == MOD_BOOKING_BO_COND_PRICEISSET) {
        \mod_booking\booking_bookit::answer_booking_option('option', $settings->id, MOD_BOOKING_STATUSPARAM_RESERVED, $user->id);
        [$id, $isavailable, $description] = $boinfo->is_available($settings->id, $user->id, true);
        if ($id == MOD_BOOKING_BO_COND_ALREADYRESERVED) {
            $bookedusers[] = $user;
        }
    } else {
        // Once there are no places anymore, we start unloading reserved places here.
        // In the parallel thread, we continue booking.
        break;
    }
    usleep(20000);
...
foreach ($bookedusers as $user) {
    ...
    $allbookedusers = \mod_booking\booking_answers\booking_answers::count_places($ba->get_usersonlist());
    if ($allbookedusers > $settings->maxanswers) {
        $errorcounter[] = "To many users on list: $allbookedusers";
    }

    \mod_booking\booking_bookit::answer_booking_option('option', $settings->id, MOD_BOOKING_STATUSPARAM_NOTBOOKED, $user->id);
    // We run through this really quickly.
    usleep(8000);
    ...
    $dbcountbooked = $DB->count_records('booking_answers', ['waitinglist' => 0]);
    $dbcountreserved = $DB->count_records('booking_answers', ['waitinglist' => 2]);
    $allbookedusers = $dbcountbooked + $dbcountreserved;
    if ($allbookedusers > $settings->maxanswers) {
        $errorcounter[] = "To many users on in DB: $allbookedusers";
    }
}

$count = $DB->count_records('booking_answers', ['waitinglist' => 0]);

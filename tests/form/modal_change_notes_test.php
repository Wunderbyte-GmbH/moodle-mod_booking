...
/**
 * Tests for the notes bulk modal of the bookings tracker.
 *
 * @package mod_booking
 * @category test
...
use mod_booking\form\optiondates\modal_change_notes;
...
/**
 * The modal changes the notes of the checked booked users - in the option
 * scope (stored in booking_answers.notes) and per session in the optiondate
 * scope (stored in booking_optiondates_answers). Both are gated by
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
final class modal_change_notes_test extends booking_advanced_testcase {
    /**
     * Option scope: checkedids are booking_answers ids; the notes of all
     * checked users are written to booking_answers.notes.
     *
     * @covers \mod_booking\form\optiondates\modal_change_notes::process_dynamic_submission
     */
    public function test_option_scope_sets_notes(): void {
        global $DB;
        ...
        $this->submit_modal([
            ...
            'notes' => 'Bulk note from tracker',
        ]);

        foreach ($answerids as $answerid) {
            $this->assertEquals(
                'Bulk note from tracker',
                $DB->get_field('booking_answers', 'notes', ['id' => $answerid]),
                "The notes of answer $answerid must be updated."
            );
        }
    }
     ...
     * the notes are stored per session in booking_optiondates_answers.
     *
     * @covers \mod_booking\form\optiondates\modal_change_notes::process_dynamic_submission
     */
    public function test_optiondate_scope_sets_notes_per_session(): void {
        global $DB;
        ...
        $this->submit_modal([
            ...
            'notes' => 'Session note from tracker',
        ]);
        ...
        foreach ($students as $student) {
            ...
            $this->assertEquals('Session note from tracker', $record->notes);
        }
    }

    /**
     * The modal is gated by managebookedusers: non-editing teachers (read-only
     * in the tracker) are rejected on construction already.
     *
     * @covers \mod_booking\form\optiondates\modal_change_notes::check_access_for_dynamic_submission
     */
    ...
    private function submit_modal(array $ajaxargs): void {
        $submitdata = modal_change_notes::mock_ajax_submit($ajaxargs);
        $form = new modal_change_notes(null, null, 'post', '', [], true, $submitdata, true);
    ...
    private function create_booked_option_with_session(): array {
        ...
        $record->text = 'Option for notes modal';
        $record->useprice = 0;
        $record->maxanswers = 5;
